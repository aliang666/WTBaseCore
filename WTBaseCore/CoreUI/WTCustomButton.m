...
@implementation WTCustomButton
+ (id)buttonWithType:(UIButtonType)buttonType withSpace:(CGFloat)space {
    WTCustomButton *button = [super buttonWithType:buttonType];
    button.space = space;
    return button;
}
...
- (void)layoutSubviews {
    ...
    switch (self.buttonStyle) {
        ...
        case WTCustomButtonImageTop: {
            ...
            break;
        case WTCustomButtonImageBottom: {
            //设置后的image显示的高度
            CGFloat imageHeight = self.frame.size.height - (2 * self.space) - labelHeight - self.padding;
            if (imageHeight > image.size.height) {
                imageHeight = image.size.height;
